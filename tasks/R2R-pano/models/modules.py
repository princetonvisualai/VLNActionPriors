...
class VariationalEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dims, ve_prior_type = False):
        super(VariationalEncoder, self).__init__()
        self.device = device
        self.mlp = build_mlp(input_dim, hidden_dims)
        self.output_dim = hidden_dims[-1]
        self.ve_prior_type = ve_prior_type
        
        self.mu  = nn.Linear(self.output_dim, self.output_dim)
        self.logvar = nn.Linear(self.output_dim, self.output_dim)

        if self.ve_prior_type == 'simple':
            self.p_mu     = nn.Parameter(torch.zeros(self.output_dim))
            self.p_logvar = nn.Parameter(torch.zeros(self.output_dim))
        elif self.ve_prior_type == 'recurrent':
            self.mu_list     = []
            self.logvar_list = []
            self.p_mu     = torch.zeros(self.output_dim).to(self.device)
            self.c_mu     = torch.zeros(self.output_dim).to(self.device)
            self.p_logvar = torch.zeros(self.output_dim).to(self.device)
            self.c_logvar = torch.zeros(self.output_dim).to(self.device)
            self.p_mu_lstm     = nn.LSTMCell(self.output_dim, self.output_dim) 
            self.p_logvar_lstm = nn.LSTMCell(self.output_dim, self.output_dim) 
    ...
    def advance_lstm(self):
        import pdb; pdb.set_trace()
        mu_avg     = torch.cat(self.mu_list, dim=1).mean(dim=1) 
        logvar_avg = torch.cat(self.logvar_list, dim=1).mean(dim=1) 
        p_mu     = self.p_mu.expand_as(mu_avg) 
        c_mu     = self.c_mu.expand_as(mu_avg) 
        p_logvar = self.p_logvar.expand_as(logvar_avg) 
        c_logvar = self.c_logvar.expand_as(logvar_avg) 
        self.p_mu, self.c_mu = self.p_mu_lstm(mu_avg, (p_mu, c_mu))
        self.p_logvar, self.c_logvar = self.p_mu_lstm(logvar_avg, (p_logvar, c_logvar))
        self.mu_list = []
        self.logvar_list = []
    ...
    def forward(self, x):
        ...
        if self.ve_prior_type == 'recurrent':
            if len(x.size()) < 3:
                mu     = mu.unsqueeze(dim=1)
                logvar = logvar.unsqueeze(dim=1)
            self.mu_list.append(mu)
            self.logvar_list.append(logvar)
            loss = self.bikld_loss(mu, logvar)
        else:
